...
namespace PillPal
{
    public partial class Win_Log_Pacjent : Form
    {
        ...
        private async void Zalogujsie_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                Pacjent authenticatedPacjent = await AuthenticatePacjentAsync(login, haslo);
                if (authenticatedPacjent != null)
                {
                    // Pomyślne logowanie, otwórz nowe okno
                    Win_Panel_Pacjent panelPacjent = new Win_Panel_Pacjent(authenticatedPacjent, this.loadingWin);
                    panelPacjent.Show();
        ...
        private async Task<Pacjent> AuthenticatePacjentAsync(string login, string haslo)
        {
