...
namespace PillPal
{
    public partial class Win_Dodaj_Recepte : Form
    {
        ...
        private string kodReceipty; // Store the generated kod receipty
        private int idReceipty; // Store the generated ID receipty
        ...
        private async Task<bool> AddReceptaAsync(int idPacjenta, int idLekarza, DateTime dataWystawienia, List<ReceptaLek> leki)
        {
            using (OracleConnection connection = new OracleConnection(ConfigurationManager.ConnectionStrings["OracleDbContext"].ConnectionString))
            {
                // Check if the receipt already exists, if not, create it
                if (string.IsNullOrEmpty(kodReceipty))
                {
                    using (OracleCommand command = new OracleCommand("ADD_RECEIPTA", connection))
                    {
                ...
                // Now add all the medicines to the created receipt
                foreach (var lek in leki)
                {
                    using (OracleCommand lekCommand = new OracleCommand("ADD_RECEIPTA_LEKI", connection))
                    {
